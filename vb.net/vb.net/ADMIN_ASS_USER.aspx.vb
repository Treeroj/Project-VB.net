...
Partial Class ADMIN_ASS_USER
    ...
    Protected Sub tbpassword_TextChanged(sender As Object, e As EventArgs)
        ...
        Select Case tbpassword.Text = strUserPassword And tbpassword.Text <> String.Empty
            Case True
                ...
                Dim cmd As New OracleCommand("select ASS_REGISUSER from NVP_ALLPROCESS_ADMIN where PASSWORD = '" & tbpassword.Text & "' and USERADMIN = '" & tbUser.Text & "' and ASS_REGISUSER = 'Y'", conn)
                conn.Open()
    ...
    Private Sub Load_gvUSER()
        ...
        Dim acc As String = "SELECT ROWID,USERID,USERNAME,PROCESS,PRODUCTS FROM NVP_ALLPROCESS_USER where PROCESS ='" & PROCESS & "'"
    ...
    Private Sub ApplyFilters()
        ...
        If Not String.IsNullOrEmpty(TextBox1.Text) Then
            filterExpressions.Add("USERNAME LIKE '%" & TextBox1.Text & "%'")
            Load_gvUSER()
        End If
        If Not String.IsNullOrEmpty(TextBox2.Text) Then
            filterExpressions.Add("USERID LIKE '%" & TextBox2.Text & "%'")
            Load_gvUSER()
        End If
    ...
    Protected Sub gvUSER_RowUpdating(sender As Object, e As GridViewUpdateEventArgs)
        ...
        Dim cmdUSERID As New OracleCommand("Select USERID,USERNAME,PROCESS,PRODUCTS FROM NVP_ALLPROCESS_USER WHERE ROWID = '" & customerId & "'", conUSERID)
        conUSERID.Open()
        ...
        Using conn As New OracleConnection(MT800DB)
            conn.Open()
            Dim acc As String = "UPDATE NVP_ALLPROCESS_USER SET USERID = :USERID, USERNAME = :USERNAME , PROCESS = :PROCESS, PRODUCTS = :PRODUCTS where ROWID = '" & customerId & "'"
            Dim cmd As New OracleCommand(acc, conn)
            cmd.Parameters.Add(":USERID", OracleDbType.Varchar2).Value = USERID
            cmd.Parameters.Add(":USERNAME", OracleDbType.Varchar2).Value = USERNAME
            cmd.Parameters.Add(":PROCESS", OracleDbType.Varchar2).Value = POSITION
            cmd.Parameters.Add(":PRODUCTS", OracleDbType.Varchar2).Value = PRODUCT
    ...
    "(ADMINCHANGE,DAY,TIME,USERID,USERNAME,PROCESS,PRODUCTS,KEYHIS,STATUT)" +
    "values" +
    "(:sADMINCHANGE,:sDAY,:sTIME,:sUSERID,:sUSERNAME,:sPROCESS,:sPRODUCTS,:sKEYHIS,:sSTATUT)"
            ...
            objCmd.Parameters.Add("@sPRODUCTS", strPRODUCTRE)
...
"(:sADMINCHANGE,:sDAY,:sTIME,:sUSERID,:sUSERNAME,:sPROCESS,:sPRODUCTS,:sKEYHIS,:sSTATUT)"
        ...
        objCmd.Parameters.Add("@sPRODUCTS", strPRODUCT)
        ...
        Using conn As New OracleConnection(MT800DB)
            conn.Open()
            Dim acc As String = "DELETE FROM NVP_ALLPROCESS_USER WHERE ROWID = '" & customerId & "'"
            Dim cmd As New OracleCommand(acc, conn)
            cmd.ExecuteNonQuery()
        End Using
        ' Close the modal
...
"(USERID,USERNAME,PROCESS,PRODUCTS)" +
"values" +
"(:sUSERID,:sUSERNAME,:sPROCESS,:sPRODUCTS)"
...
"(:sADMINCHANGE,:sDAY,:sTIME,:sUSERID,:sUSERNAME,:sPROCESS,:sPRODUCTS,:sKEYHIS,:sSTATUT)"
            ...
            objCmdchk.Parameters.Add("@sPROCESS", "ASSEMBLY")
            objCmdchk.Parameters.Add("@sPRODUCTS", ddlLinePD.SelectedItem.Value)
    ...
    Protected Sub tbEmployeeId_TextChanged(sender As Object, e As EventArgs) Handles tbEmployeeId.TextChanged
        ...
        Else
            ...
            Dim cmdChkid As New OracleCommand("select USERNAME from NVP_ALLPROCESS_USER where USERNAME = '" & strtbEmployeeId & "' and PRODUCTS = 'PSMOD' and  PRODUCTS = 'DCMOD'", connChkid)
            connChkid.Open()
    ...
    Private Sub Load_gvHIS()
        Dim conn As New OracleConnection(MT800DB)
        Dim cmd As New OracleCommand()
        Dim acc As String = "SELECT ADMINCHANGE,DAY,TIME,USERID,USERNAME,PROCESS,PRODUCTS,KEYHIS,STATUT FROM NVP_ALLPROCESS_LOG_CHANGEUSER where  PROCESS ='" & PROCESS & "'"
