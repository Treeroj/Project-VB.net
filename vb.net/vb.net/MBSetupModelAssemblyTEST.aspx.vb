...
Partial Class MBSetupModelAssemblyTEST
                ...
                Case True
                    ...
                    If Session("RSQR") = "QR" Then
                        ...
                        If strMat = Session("RSMat") Then
                            ...
                            Dim cmdReelno As New OracleCommand("select REELNO,MODEL,LOTNO from NVP_ALLPROCESS_HIS where MODEL = '" & tbModel.Text & "' and REELNO = '" & Session("RSReelno") & "' ", conReelno)
                            conReelno.Open()
                            ...
                            ElseIf strReelno = Session("RSReelno") And strLot <> String.Empty And tbLot.Text <> strLot Then
                                MesgBox("หมายเลข ReelNo " & Session("RSReelno") & " ถูกยิงใน Lot " & strLot & " เรียบร้อยแล้ว กรุณาตรวจสอบและทำการยิงลาเบลใหม่")
...
"(USERID,USERNAME,DAY,TIME,HISERROR,MODEL,LOT,PRODUCTS,PROCESS)" +
"values" +
"(:sUSERID,:sUSERNAME,:sDAY,:sTIME,:sHISERROR,:sMODEL,:sLOT,:sPRODUCTS,:sPROCESS)"
...
"(:sUSERID,:sUSERNAME,:sDAY,:sTIME,:sMODEL,:sLOTNO,:sMATERIAL,:sINVOICE,:sMATERIALLOT,:sQTY,:sREELNO,:sLINE,:sPRODUCTS,:sPROCESS)"
            ...
            objConn.Open()
            Dim objCmd As New OracleCommand(strSQL, objConn)
            objCmd.Parameters.Add("@sUSERNAME", lbshowCode.InnerText)
            objCmd.Parameters.Add("@sUSERNAME", lbshowFRONTENDLINE.InnerText)
            objCmd.Parameters.Add("@sDAY", CStr(Format(Now, "dd-MMM-yyyy")))
            objCmd.Parameters.Add("@sTIME", CStr(Format(Now, "HH:mm:ss")))
            objCmd.Parameters.Add("@MODEL", tbModel.Text)
